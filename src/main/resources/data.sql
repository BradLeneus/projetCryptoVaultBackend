insert into crypto (name, price, ticker) values ( 'Bitcoin', 80000, 'Btc');
insert into crypto (name, price, ticker) values ('Etherum', 4000, 'Eth');
insert into crypto (name, price, ticker)values  ('BittensorTao', 10000, 'Tao');
insert into crypto (name, price, ticker) values ('Solana', 180, 'Sol');
insert into crypto (name, price, ticker) values ( 'ShibaInu', 0.001, 'Shiba');


insert into customer (email , fname, lname) values("aa@gmail.com", "sam", "$2a$10$DqJ961drU95HOyHBWsJFSeEWl5sdZSi41ojbyH9leFY3RNtpCKbJu");
insert into customer (email , fname, lname) values("adad@gmail.com", "sas", "$2a$10$9obbP9ZtY.w9mEO/74CAreBk0uMPzvMLvR0cPTLVIXdSjGToXRR4q");
insert into customer (email , fname, lname) values("toto@gmail.com", "yui", "$2a$10$wd7inYw7bwmdUggS3w/ORu1FKB7XJmwYhnyUpGb8HWXPUv1ZGuqj2");
insert into customer (email , fname, lname) values("tata@gmail.com", "dasdads", "$2a$10$wd7inYw7bwmdUggS3w/ORu1FKB7XJmwYhnyUpGb8HWXPUv1ZGuqj2");
insert into customer (email , fname, lname) values("asd@gmail.com", "boss", "$2a$10$SpHeLwA7SljnPoNBgA4uFuCs0FGGmUMXF0/Q2G03QslOu8yyXCGUy");

insert into wallet(idcrypto_id, qty, customer_id) values (1, 1, 1);
insert into wallet(idcrypto_id, qty, customer_id) VALUES (2, 10, 1);
insert into wallet(idcrypto_id, qty, customer_id) VALUES (3, 4, 2);
insert into wallet(idcrypto_id, qty, customer_id) VALUES (1, 2, 2);